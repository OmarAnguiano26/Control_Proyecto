...
#define MAGNETOMETER_CALIBON (1)
#define MAGNETOMETER_RAY_TEST (1)

#define LEVEL 1000
#define MAX_LEVEL 2*LEVEL
#define STEP_LEVEL 5	// 0.25%

#define INITPWM 3
#define MAXPWM 10
#define MINPWM 1

#define INIT_LEVEL		(MAX_LEVEL - (MAX_LEVEL*INITPWM)/100)
#define MAXIM_VEL 		(MAX_LEVEL - (MAX_LEVEL*MAXPWM)/100)
#define MINIM_VEL 		(MAX_LEVEL - (MAX_LEVEL*MINPWM)/100)
...
//-----------------------------------------------------------------------
//Más variables de el controlador
//-----------------------------------------------------------------------
/* Whether the SW button is pressed */
volatile bool g_ButtonPress3 = false;
volatile bool g_ButtonPress2 = false;
volatile uint32_t g_systickCounter;
uint32_t level = INIT_LEVEL;
...
const float Jy = 0.02;
const float Jp = 0.02;
const float by = 0.005;
const float bp = 0.005;
const float Kpp = 1;
const float Kyy = 1;
const float Kyp = 0.1;
const float Kpy = 0.1;

float Y1r, Y2r, dY1r, dY2r, ddY1r, ddY2r;

float f3, f4, G1, G2, G3, G4;

float X1, X2, X3, X4;

float e1a, de1a, auxa, dauxa;
float e1b, de1b, auxb, dauxb;
float e2a, e2b, U1, U2;

float x3ref, dx3ref, x4ref, dx4ref;

const ftm_config_t FTM_1_config = {
  .prescale = kFTM_Prescale_Divide_128,
  .bdmMode = kFTM_BdmMode_0,
  .pwmSyncMode = kFTM_SoftwareTrigger,
  .reloadPoints = kFTM_Chnl0Match,
...
int main(void)
{
    ...
    FTM_Init(FTM0, &FTM_1_config);
    FTM_SetupPwm(FTM0, FTM_1_pwmSignalParams, sizeof(FTM_1_pwmSignalParams) / sizeof(ftm_chnl_pwm_signal_param_t), kFTM_EdgeAlignedPwm, 50U, CLOCK_GetFreq(kCLOCK_BusClk));
    ...
    /**Referencias*/
	Y1r = 3;  dY1r = 0;     ddY1r = 0;
	Y2r = 3;  dY2r = 0;    ddY2r = 0;
	float duty = 5;
    ...
    {        /* In ISR Mode we do not need to check Data Ready Register.
        ...
        if(pitIsrFlag)
        {
            ...
            /**********************Controlador************************************/
            	X1 = ((magx-MagneticFieldX)/100);
                X2 = ((magy-MagneticFieldY)/100);
                X3 = 0;//((magz-MagneticFieldZ)/100);
                X4 = 0;
        		//Funciones del Sistema
        		f3 = (-m * g * l * cos(X1) - bp * X3
        				- m * l * l * sin(X1) * cos(X1) * X4 * X4) / (Jp + m / l * l);
        		f4 = (-by * X4 + 2 * m * l * l * sin(X1) * cos(X1) * X4 * X3)
        				/ (Jy + m * l * l * cos(X1) * cos(X1));
        		G1 = Kpp / (Jp + m * l * l);
        		G2 = Kpy / (Jp + m * l * l);
        		G3 = Kyp / (Jy + m * l * l * cos(X1) * cos(X1));
        		G4 = Kyy / (Jy + m * l * l * cos(X1) * cos(X1));
        		...
        		x3ref=auxa;   x4ref=auxb;
        		dx3ref=dauxa;   dx4ref=dauxb;

        		//2do Bloque
        		e2a = x3ref-X3;
        		// de2a = dx3ref-f3-G*U1; = K2*e2a
        		U1=(dx3ref-f3-K2*e2a)/(G1 + G2);// + (dx3ref-f3-K2*e2a)/G2;

        		e2b = x4ref-X4;
        		// de2 = dx4ref-f3-G*U2; = K2*e2b
        		U2=(dx4ref-f4-K2*e2b)/(G3 + G4);// + (dx4ref-f4-K2*e2b)/G4;

        		/**TODO Modificar para control**********************************/
            	GPIO_PortSet(GPIOB, 1U << 2);
            	SysTick_DelayTicks(MAX_LEVEL - level);
            	GPIO_PortClear(GPIOB, 1U << 2);
            	SysTick_DelayTicks(level);
